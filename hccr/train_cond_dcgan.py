...
#reshape and shuffle
trX = np.reshape(trX , (-1 , npx , npx)).astype(np.float32)
vaX = np.reshape(vaX , (-1 , npx , npx)).astype(np.float32)
teX = np.reshape(teX , (-1 , npx , npx)).astype(np.float32)
trY = np.reshape(trY , (-1 , code_len)).astype(np.float32)
vaY = np.reshape(vaY , (-1 , code_len)).astype(np.float32)
teY = np.reshape(teY , (-1 , code_len)).astype(np.float32)
trX,trY = shuffle(trX , trY)

def transform(X):
    return X.reshape(-1, nc, npx, npx)
...
gw  = gifn((nz+code_len, ngfc), 'gw')
gw2 = gifn((ngfc+code_len, ngf*2*7*7), 'gw2')
gw3 = gifn((ngf*2+code_len, ngf, 5, 5), 'gw3')
gwx = gifn((ngf+code_len, nc, 5, 5), 'gwx')

dw  = difn((ndf, nc+code_len, 5, 5), 'dw')
dw2 = difn((ndf*2, ndf+code_len, 5, 5), 'dw2')
dw3 = difn((ndf*2*7*7+code_len, ndfc), 'dw3')
dwy = difn((ndfc+code_len, 1), 'dwy')

gen_params = [gw, gw2, gw3, gwx]
discrim_params = [dw, dw2, dw3, dwy]

def gen(Z, Y, w, w2, w3, wx): #Z,Y->X
...
#trX_vis = np.asarray([[trX[i] for i in py_rng.sample(tr_idxs[trY==y], 20)] for y in range(10)]).reshape(200, -1) #TODO
trX_vis = trX[:200,:,:].reshape(200,-1)
...
#sample_ymb = floatX(OneHot(np.asarray([[i for _ in range(20)] for i in range(10)]).flatten(), ny)) #TODO
sample_ymb = np.zeros((200 , code_len))
for i in range(200):
    sample_ymb[i,:] = mapping[i%ny]
sample_ymb = floatX(sample_ymb)
...
def gen_samples(n, nbatch=128):
    ...
    for i in range(n/nbatch):
        #ymb = floatX(OneHot(np_rng.randint(0, 10, nbatch), ny))  #TODO
        ymb = np.ones((nbatch , code_len))
        for j in range(nbatch):
            ymb[j,:] = mapping[np.random.randint(0,ny)]
        ymb = floatX(ymb)
        zmb = floatX(np_rng.uniform(-1., 1., size=(nbatch, nz)))
        xmb = _gen(zmb, ymb)
        samples.append(xmb)
        #labels.append(np.argmax(ymb, axis=1))  #TODO
        labels.append(ymb)
        n_gen += len(xmb)
    n_left = n-n_gen
    #ymb = floatX(OneHot(np_rng.randint(0, 10, n_left), ny)) #TODO
    ymb = np.ones((n_left, code_len))
